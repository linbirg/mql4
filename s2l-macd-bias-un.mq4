...
#property copyright   "2015-2025, Obug Software Corp."
...
int CurrentStop2MA()
{
   ...
   for(int cnt=1;cnt<MASwitchCount;cnt++)
   {
      ...
      if(OrderType()==OP_BUY)
      {
         if(stopLost>=MA)maTrendPeriod=MATrendPeriod*MASwitchStep*cnt;
         if(stopLost<MA)
         {  
...
{
   if(isNearByMAAndFarAway()
      &&isMaUp(PERIOD_M5,MAOpenLevel,getNearByMa())
     )
     {
...
{
   if(isNearByMAAndFarAway()
      &&isMaDown(PERIOD_M5,MAOpenLevel,getNearByMa()))
     {
...
void CheckForOpen()
{
   ...
   if(CalculateCurrentOrders()>0)longPosition=true;
   if(CalculateCurrentOrders()<0)shortPosition=true;
...
double latestOpenPrice()
{
   ...
   for(int i=0;i<OrdersTotal();i++)
   {
      if(SelectTradeOrderByPos(i))return OrderOpenPrice();
   }
...
bool stopLossCoveredlatestOpenPrice()
{
   ...
   {
      // 
      if(lowestStop()>latestOpenPrice())return true;//+TrailingStop*Point
      return false;
   }
...
double calcHighest()
{
   ...
   {
      if(high == 0) high = Close[i];
     
      if(high<Close[i])high=Close[i];
   }
...
double calcLowest()
{
   ...
   for(int i=1;i<CheckPeriod/2;i++)
   {
      if(low == 0)low = Close[i];
      
      if(low>Close[i])low = Close[i];
   }
...
int chooseMa4HPeriod()
{
   ...
   if(currStopMA>MATrendPeriodFast*16)MAPeriod=MATrendPeriod;
   if(currStopMA>MATrendPeriod*16)MAPeriod=2*MATrendPeriod;
